...
int fork() {   // the syscall of fork is 2
	return syscall(2, 0, 0, 0, 0, 0, 0);
}

int sleep(unsigned int time) {   // the syscall of sleep is 7
	return syscall(7, 0, 0, time, 0, 0, 0);
}

int exit(int sig) {   // the syscall of exit is 1
	return syscall(1, 0, 0, sig, 0, 0, 0);
}

int createsem() {
	return syscall(10, 0, 0, 0, 0, 0, 0);
}

int destroysem() {
	return syscall(11, 0, 0, 0, 0, 0, 0);
}
